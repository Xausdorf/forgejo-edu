...
func RegisterRoutes(m *web.Route, middlewares ...any) {
	m.Group("/edu", func() {
		...
		m.Group("/teacher", func() {
			...
			m.Get("/assignments/{id}/submissions/{subID}", SubmissionReview)
			m.Post("/assignments/{id}/submissions/{subID}/approve", ApproveSubmissionPost)
			m.Post("/assignments/{id}/submissions/{subID}/merge", MergeSubmissionPost)
			m.Post("/assignments/{id}/submissions/{subID}/comment", CommentSubmissionPost)
			m.Post("/assignments/{id}/submissions/{subID}/reset-approval", ResetApprovalPost)
			...
			m.Group("/courses", func() {
				...
				m.Get("/{id}/sync", CourseSyncPage)
				m.Post("/{id}/sync/start", StartCourseSyncPost)
				m.Get("/{id}/sync/status", CourseSyncStatus)
				m.Post("/{id}/sync/{taskID}/merge-all", MergeAllCourseSyncPost)
				m.Post("/{id}/sync/{taskID}/merge/{prID}", MergeOneCourseSyncPost)
